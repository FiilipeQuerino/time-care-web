...
export const DashboardPage = () => {
    ...
    return (
      <div className="bg-white rounded-[2rem] border border-slate-100 shadow-sm p-6">
        ...
        <p className="text-xs text-sky-700 bg-sky-50 border border-sky-100 rounded-lg px-3 py-2 mb-4">
          Use no celular para importar na agenda e salvar os contatos automaticamente.
        </p>

        <div className="grid grid-cols-1 md:grid-cols-3 gap-3 mb-5">
          <Input label="Buscar cliente" placeholder="Nome, e-mail ou telefone" value={clientSearch} onChange={(e) => setClientSearch(e.target.value)} />
          <div className="flex flex-col gap-1.5">
            <label className="text-sm font-semibold text-slate-700 ml-1">Status</label>
            <select value={clientStatusFilter} onChange={(e) => setClientStatusFilter(e.target.value as 'all' | 'active' | 'inactive')} className="w-full px-4 py-3 rounded-xl border-2 border-slate-100 bg-slate-50/50 focus:border-pink-500 focus:bg-white focus:outline-none transition-all duration-200 text-slate-800">
          ...
          <div className="flex items-end">
            <Button size="md" variant="outline" className="w-full" onClick={() => { setClientSearch(''); setClientStatusFilter('all'); }}>Limpar filtros</Button>
          </div>
        </div>
        ...
        {filteredClients.length === 0 ? (
          ...
          <div className="space-y-3">
            {filteredClients.map((client) => (
              <div key={client.clientId} className="border border-slate-100 rounded-xl px-4 py-3 flex flex-col gap-3">
                <div className="flex flex-col gap-1 sm:flex-row sm:items-center sm:justify-between">
                  ...
                  <span className={`text-xs font-semibold px-2.5 py-1 rounded-full w-fit border ${client.isActive ? 'bg-emerald-100 text-emerald-700 border-emerald-200' : 'bg-amber-100 text-amber-700 border-amber-200'}`}>
                    {client.isActive ? 'Ativo' : 'Inativo'}
                  </span>
                </div>
                <div className="flex flex-wrap gap-2">
                  <Button size="sm" variant="outline" icon={UserRoundPen} onClick={() => openEditClient(client)}>Editar</Button>
                  <Button size="sm" icon={client.isActive ? UserX : UserCheck} className={client.isActive ? 'bg-amber-500 hover:bg-amber-600 text-white shadow-amber-200' : 'bg-emerald-600 hover:bg-emerald-700 text-white shadow-emerald-200'} onClick={() => void handleToggleClientStatus(client)} isLoading={statusLoadingId === client.clientId} disabled={statusLoadingId === client.clientId}>
  ...
  const renderProceduresSection = () => {
    ...
    return (
      <div className="space-y-6">
        <div className="bg-white rounded-[2rem] border border-slate-100 shadow-sm p-6">
          <h3 className="text-lg font-bold text-slate-800 mb-4">Cadastrar procedimento</h3>
          <form className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-5 gap-3" onSubmit={handleCreateProcedure}>
            <Input label="Nome" placeholder="Ex: Brow Lamination" value={procedureName} onChange={(e) => setProcedureName(e.target.value)} required />
            <Input label="Preco sugerido" type="number" min="1" step="0.01" placeholder="0,00" value={procedurePrice} onChange={(e) => setProcedurePrice(e.target.value)} required />
            <Input label="Duracao (min)" type="number" min="1" placeholder="60" value={procedureDuration} onChange={(e) => setProcedureDuration(e.target.value)} required />
            <div className="flex flex-col gap-1.5">
              <label className="text-sm font-semibold text-slate-700 ml-1">Categoria</label>
              <select value={procedureCategory} onChange={(e) => setProcedureCategory(Number(e.target.value))} className="w-full px-4 py-3 rounded-xl border-2 border-slate-100 bg-slate-50/50 focus:border-pink-500 focus:bg-white focus:outline-none transition-all duration-200 text-slate-800">
            ...
            <div className="flex items-end">
              <Button type="submit" className="w-full" isLoading={isCreatingProcedure} disabled={isCreatingProcedure}>Cadastrar</Button>
            </div>
          </form>
        </div>
        ...
        <div className="bg-white rounded-[2rem] border border-slate-100 shadow-sm p-6">
          <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-3 mb-4">
            <h3 className="text-lg font-bold text-slate-800">Lista de procedimentos</h3>
            <div className="w-full md:w-[320px]">
              <Input placeholder="Filtrar por nome/categoria" value={procedureSearch} onChange={(e) => setProcedureSearch(e.target.value)} />
            </div>
          </div>

          {filteredProcedures.length === 0 ? (
            <p className="text-slate-500">Nenhum procedimento encontrado.</p>
          ) : (
            <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-4">
              {filteredProcedures.map((procedure) => (
                <div key={procedure.procedureId} className="border border-slate-100 rounded-2xl p-4 bg-gradient-to-b from-white to-slate-50">
                  <div className="flex items-center justify-between mb-2">
                    <p className="font-bold text-slate-800">{procedure.name}</p>
                    <span className="text-xs px-2 py-1 rounded-full bg-pink-100 text-pink-700 border border-pink-200">{getProcedureCategoryLabel(procedure.category)}</span>
                  </div>
                  <p className="text-sm text-slate-500 mb-1">Preco sugerido</p>
                  <p className="text-lg font-bold text-slate-900 mb-2">{formatCurrency(procedure.suggestedPrice)}</p>
                  <p className="text-sm text-slate-500">Duracao estimada: <span className="font-semibold text-slate-700">{procedure.estimatedDurationInMinutes} min</span></p>
                </div>
              ))}
            </div>
          )}
  ...
  const shouldShowPrimaryAction = activeSection !== 'relatorios' && activeSection !== 'config';
